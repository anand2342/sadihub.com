...
import { dbGetProfile, dbGetFamilyById, dbGetUserRoles, dbUpsertProfile, dbInsertRole, dbCreateFamily, dbGetFamilyByCode, dbGetFamilyBySlugOrName, dbUpdateProfile, dbCheckSuperAdminExists, } from '../lib/supabaseApi';
...
const SupabaseAuthProvider = ({ children }) => {
    ...
    const buildSession = async (userId, email) => {
        try {
    ...
    const loginWithFamily = async (_familyCodeOrName) => {
    ...
    const joinFamily = async (params) => {
        ...
        try {
            ...
            if (!family)
                family = await dbGetFamilyBySlugOrName(cleanInput);
            if (!family) {
                setLoading(false);
    ...
    const refreshSession = () => {
        if (session)
            buildSession(session.user_id, session.email).then(setSession);
    };
...
const LocalAuthProvider = ({ children }) => {
    ...
    const loginWithFamily = async (familyCodeOrName) => {
        const store = getStore();
        const cleanInput = familyCodeOrName.trim();
        const slug = slugify(cleanInput);
        const family = store.families.find(f => f.family_code.toUpperCase() === cleanInput.toUpperCase() ||
            f.slug === slug || f.name.toLowerCase() === cleanInput.toLowerCase());
        if (!family)
            return { success: false, message: `No family found matching "${familyCodeOrName}".` };
    ...
    const joinFamily = async (params) => {
        ...
        const family = store.families.find(f => f.family_code.toUpperCase() === cleanInput.toUpperCase() ||
            f.slug === slug || f.name.toLowerCase() === cleanInput.toLowerCase());
        if (!family)
            return { success: false, message: `Family matching "${params.familyName}" not found.` };
    ...
    const createFamily = async (params) => {
        ...
        if (store.families.some(f => f.slug === slug))
            return { success: false, message: `Family "${params.familyName}" already exists!` };
        if (store.authUsers.some(u => u.email.toLowerCase() === params.adminEmail.trim().toLowerCase()))
    ...
    const claimSuperAdmin = async () => {
        ...
        if (!session)
            return { success: false, message: 'You must be logged in.' };
        if (store.userRoles.some(r => r.role === 'super_admin'))
            return { success: false, message: 'Super Admin already exists!' };
        ...
        if (pi >= 0)
            store.profiles[pi].status = 'approved';
        saveStore(store);
        loadSessionFromStore();
        return { success: true };
    };
    ...
    const switchFamily = (familyId) => {
        ...
        if (session.profile) {
            const pi = store.profiles.findIndex(p => p.id === session.user_id);
            if (pi >= 0)
                store.profiles[pi].family_id = familyId;
        }
        saveStore(store);
        loadSessionFromStore();
    };
