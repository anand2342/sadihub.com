...
export async function dbGetFamilyBySlugOrName(input) {
...
export async function dbCreateFamily(f) {
...
export async function dbGetAllFamilies() {
...
export async function dbGetFamilyProfiles(familyId) {
    const { data } = await supabase
        .from('profiles')
        .select('*')
...
export async function dbUpdateMemberStatus(profileId, status) {
    const { error } = await supabase.from('profiles').update({ status }).eq('id', profileId);
    if (error)
        throw new Error(error.message);
}
export async function dbDeleteProfile(profileId) {
...
// USER ROLES
// ─────────────────────────────────────────────
export async function dbGetUserRoles(userId) {
    const { data } = await supabase.from('user_roles').select('*').eq('user_id', userId);
    return (data || []).map(mapRole);
}
export async function dbInsertRole(role) {
...
export async function dbCheckSuperAdminExists() {
...
// EVENTS
// ─────────────────────────────────────────────
export async function dbGetEvents(familyId) {
    const { data } = await supabase
        .from('events')
        .select('*')
        .eq('family_id', familyId)
...
// GALLERY PHOTOS
// ─────────────────────────────────────────────
export async function dbGetPhotos(familyId, userId) {
    ...
    const likedPhotoIds = new Set((likesRes.data || []).map((l) => l.photo_id));
...
export async function dbUploadPhotoFile(familyId, userId, file, caption, userFullName) {
...
export async function dbCountUserPhotos(familyId, userId) {
    const { count } = await supabase
        .from('gallery_photos')
        .select('*', { count: 'exact', head: true })
        .eq('family_id', familyId)
...
export async function dbToggleLike(photoId, userId) {
...
export async function dbAddComment(photoId, userId, comment, userName) {
    const { data, error } = await supabase
        .from('photo_comments')
...
export async function dbDeletePhoto(photoId, storagePath) {
...
// WISHES
// ─────────────────────────────────────────────
export async function dbGetWishes(familyId) {
    const { data } = await supabase
        .from('wishes')
        .select('*')
        .eq('family_id', familyId)
...
export async function dbInsertWish(w) {
    const { data, error } = await supabase
        .from('wishes')
...
export async function dbDeleteWish(wishId) {
    await supabase.from('wishes').delete().eq('id', wishId);
}
// ─────────────────────────────────────────────
// VIDEOS
// ─────────────────────────────────────────────
export async function dbGetVideos(familyId) {
...
// ─────────────────────────────────────────────
export async function dbUploadAvatar(userId, file) {
    const ext = file.name.split('.').pop() || 'jpg';
...
// ─────────────────────────────────────────────
export async function dbUploadCoupleImage(familyId, role, file) {
    const ext = file.name.split('.').pop() || 'jpg';
...
function mapRole(d) {
    return { id: d.id, user_id: d.user_id, family_id: d.family_id, role: d.role, created_at: d.created_at };
}
function mapWedding(d) {
    return {
