...
export async function isJavaBackendAvailable() {
    try {
...
export async function javaLogin(email, password) {
...
export async function javaGetWishes(familyId) {
    const res = await fetch(`${JAVA_API_BASE_URL}/wishes/${familyId}`);
    if (!res.ok)
        throw new Error('Failed to fetch wishes');
    return res.json();
}
export async function javaAddWish(familyId, wish) {
    const res = await fetch(`${JAVA_API_BASE_URL}/wishes/${familyId}`, {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
    ...
    if (!res.ok)
        throw new Error('Could not submit wish');
    return res.json();
}
export async function javaApproveMember(profileId) {
    const res = await fetch(`${JAVA_API_BASE_URL}/admin/members/${profileId}/approve`, {
        method: 'POST',
    });
    if (!res.ok)
        throw new Error('Could not approve member');
    return res.json();
}
