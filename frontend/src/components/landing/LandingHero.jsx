...
export const LandingHero = ({ onOpenAuth, onOpenBootstrap }) => {
    return (<div className="relative min-h-[90vh] flex items-center justify-center overflow-hidden pt-20 pb-16 px-4">
      ...
      <div className="relative max-w-5xl mx-auto text-center z-10 space-y-8">
        ...
        <div className="flex flex-col sm:flex-row items-center justify-center gap-4 pt-4">
          <button onClick={() => onOpenAuth('create')} className="w-full sm:w-auto px-8 py-4 rounded-xl bg-gold-gradient font-medium text-base text-white shadow-xl hover:opacity-95 transition-all transform hover:-translate-y-0.5 flex items-center justify-center gap-2 cursor-pointer gold-shadow">
            <Heart className="w-5 h-5 fill-current"/>
            <span>Create Family Wedding</span>
            <ArrowRight className="w-4 h-4"/>
          </button>

          <button onClick={() => onOpenAuth('join')} className="w-full sm:w-auto px-8 py-4 rounded-xl border-2 border-[var(--border-gold)] bg-[var(--bg-card)] text-[var(--text-primary)] font-medium text-base hover:bg-[var(--bg-accent)] transition-all transform hover:-translate-y-0.5 flex items-center justify-center gap-2 cursor-pointer shadow-sm">
            <Users className="w-5 h-5 text-[var(--gold-dark)] dark:text-[var(--gold-primary)]"/>
            <span>Join Your Family</span>
          </button>

          <button onClick={() => onOpenAuth('signin')} className="w-full sm:w-auto px-8 py-4 rounded-xl border-2 border-[var(--border-gold)] bg-[var(--bg-card)] text-[var(--text-primary)] font-medium text-base hover:bg-[var(--bg-accent)] transition-all transform hover:-translate-y-0.5 flex items-center justify-center gap-2 cursor-pointer shadow-sm">
        ...
        {onOpenBootstrap && (<div className="pt-6">
            <button onClick={onOpenBootstrap} className="text-xs text-[var(--text-muted)] hover:text-[var(--gold-dark)] dark:hover:text-[var(--gold-primary)] underline transition-colors">
