...
interface BootstrapSuperAdminProps {
  onBack: () => void;
  onToast: (msg: string, type: 'success' | 'error' | 'info') => void;
}

export const BootstrapSuperAdmin: React.FC<BootstrapSuperAdminProps> = ({ onBack, onToast }) => {
  ...
  return (
    <div className="min-h-[80vh] flex items-center justify-center p-4">
      <LuxuryCard className="max-w-lg w-full text-center space-y-6">
        ...
        {exists ? (
          <div className="p-4 rounded-xl bg-amber-500/10 border border-amber-500/30 text-amber-600 dark:text-amber-400 text-xs sm:text-sm flex items-start gap-3 text-left">
          ...
          <div className="p-4 rounded-xl bg-blue-500/10 border border-blue-500/30 text-blue-600 dark:text-blue-400 text-xs sm:text-sm text-left">
            Please log in or create an account first to claim Super Admin access.
          </div>
        ) : (
          <div className="p-4 rounded-xl bg-emerald-500/10 border border-emerald-500/30 text-emerald-700 dark:text-emerald-300 text-xs sm:text-sm text-left space-y-2">
        ...
        <div className="flex flex-col sm:flex-row items-center justify-center gap-3 pt-4 border-t border-[var(--border-subtle)]">
          <button
            onClick={onBack}
            className="w-full sm:w-auto px-6 py-2.5 rounded-xl border border-[var(--border-subtle)] text-[var(--text-secondary)] font-medium text-xs hover:bg-[var(--bg-accent)] transition-all flex items-center justify-center gap-1.5 cursor-pointer"
          >
          ...
          {!exists && session && (
            <button
              onClick={handleClaim}
              disabled={loading}
              className="w-full sm:w-auto px-8 py-2.5 rounded-xl bg-gold-gradient text-white font-medium text-xs shadow-md hover:opacity-95 transition-all flex items-center justify-center gap-1.5 cursor-pointer"
            >
