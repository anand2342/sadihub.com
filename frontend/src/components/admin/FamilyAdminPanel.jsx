...
import { UserCheck, Heart, Trash2, Check, X, Plus, Save, Sparkles, UploadCloud, Clock } from 'lucide-react';
...
export const FamilyAdminPanel = ({ wedding, members, events, wishes, photos, onUpdateWedding, onAddEvent, onDeleteEvent, onApproveMember, onRejectMember, onDeleteWish, onDeletePhoto, onToast, onNavigate }) => {
    ...
    return (<div className="space-y-8 animate-fade-in">
      ...
      <div className="flex items-center gap-2 border-b border-[var(--border-subtle)] pb-2 overflow-x-auto no-scrollbar">
        <button onClick={() => setActiveTab('wedding')} className={`px-4 py-2.5 rounded-xl text-xs font-bold transition-all cursor-pointer whitespace-nowrap shrink-0 ${activeTab === 'wedding'
        ...
        <button onClick={() => setActiveTab('members')} className={`px-4 py-2.5 rounded-xl text-xs font-bold transition-all cursor-pointer relative whitespace-nowrap shrink-0 ${activeTab === 'members'
            ? 'bg-gold-gradient text-white shadow'
            : 'border border-[var(--border-gold)] bg-[var(--bg-card)] text-[var(--text-secondary)]'}`}>
          Manage Members ({members.length})
          {pendingMembers.length > 0 && (<span className="ml-1.5 px-1.5 py-0.5 rounded-full bg-amber-500 text-white text-[9px] font-bold">
              {pendingMembers.length}
            </span>)}
        </button>

        <button onClick={() => setActiveTab('events')} className={`px-4 py-2.5 rounded-xl text-xs font-bold transition-all cursor-pointer whitespace-nowrap shrink-0 ${activeTab === 'events'
            ? 'bg-gold-gradient text-white shadow'
            : 'border border-[var(--border-gold)] bg-[var(--bg-card)] text-[var(--text-secondary)]'}`}>
          Manage Events ({events.length})
        </button>

        <button onClick={() => setActiveTab('moderation')} className={`px-4 py-2.5 rounded-xl text-xs font-bold transition-all cursor-pointer whitespace-nowrap shrink-0 ${activeTab === 'moderation'
            ? 'bg-gold-gradient text-white shadow'
            : 'border border-[var(--border-gold)] bg-[var(--bg-card)] text-[var(--text-secondary)]'}`}>
          Content Moderation
        </button>
      ...
      {activeTab === 'wedding' && (<form onSubmit={handleSaveWedding} className="space-y-8">
          {/* GROOM SECTION FIRST */}
          <LuxuryCard className="space-y-4 border-2 border-[var(--border-gold)] gold-shadow">
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
              <div>
                ...
                <input type="text" required value={groomName} onChange={e => setGroomName(e.target.value)} placeholder="e.g. Rahul Kapoor" className="w-full px-3 py-2.5 rounded-xl border border-[var(--border-gold)] bg-[var(--bg-primary)] text-xs text-[var(--text-primary)] focus:outline-none"/>
              </div>

              <div>
                <label className="block text-xs font-semibold text-[var(--text-secondary)] mb-1">Groom Photo *</label>
                <div className="flex flex-col gap-2">
                  {groomImage && (<img src={groomImage} alt="Groom Preview" className="w-20 h-20 rounded-xl object-cover border-2 border-[var(--border-gold)]"/>)}
              ...
              <div>
                ...
                <input type="text" value={groomFather} onChange={e => setGroomFather(e.target.value)} placeholder="Shri Rajeev Kapoor" className="w-full px-3 py-2.5 rounded-xl border border-[var(--border-gold)] bg-[var(--bg-primary)] text-xs text-[var(--text-primary)] focus:outline-none"/>
              </div>

              <div>
                <label className="block text-xs font-semibold text-[var(--text-secondary)] mb-1">Groom Mother Name</label>
                <input type="text" value={groomMother} onChange={e => setGroomMother(e.target.value)} placeholder="Smt. Sunita Kapoor" className="w-full px-3 py-2.5 rounded-xl border border-[var(--border-gold)] bg-[var(--bg-primary)] text-xs text-[var(--text-primary)] focus:outline-none"/>
              </div>
            </div>

            <div>
              <label className="block text-xs font-semibold text-[var(--text-secondary)] mb-1">Groom Biography / Details</label>
              <textarea rows={2} value={groomBio} onChange={e => setGroomBio(e.target.value)} placeholder="A short note about the Groom..." className="w-full px-3 py-2 rounded-xl border border-[var(--border-gold)] bg-[var(--bg-primary)] text-xs text-[var(--text-primary)] focus:outline-none"/>
            </div>
          </LuxuryCard>
          ...
          <LuxuryCard className="space-y-4 border-2 border-[var(--border-gold)] gold-shadow">
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
              <div>
                ...
                <input type="text" required value={brideName} onChange={e => setBrideName(e.target.value)} placeholder="e.g. Ananya Verma" className="w-full px-3 py-2.5 rounded-xl border border-[var(--border-gold)] bg-[var(--bg-primary)] text-xs text-[var(--text-primary)] focus:outline-none"/>
              </div>

              <div>
                <label className="block text-xs font-semibold text-[var(--text-secondary)] mb-1">Bride Photo *</label>
                <div className="flex flex-col gap-2">
                  {brideImage && (<img src={brideImage} alt="Bride Preview" className="w-20 h-20 rounded-xl object-cover border-2 border-[var(--border-gold)]"/>)}
                  <label className="px-4 py-2.5 rounded-xl bg-gold-gradient text-white font-semibold text-xs cursor-pointer hover:opacity-90 flex items-center justify-center gap-2 shrink-0 gold-shadow">
                    <UploadCloud className="w-4 h-4"/>
              ...
              <div>
                ...
                <input type="text" value={brideFather} onChange={e => setBrideFather(e.target.value)} placeholder="Shri Mahendra Verma" className="w-full px-3 py-2.5 rounded-xl border border-[var(--border-gold)] bg-[var(--bg-primary)] text-xs text-[var(--text-primary)] focus:outline-none"/>
              </div>

              <div>
                <label className="block text-xs font-semibold text-[var(--text-secondary)] mb-1">Bride Mother Name</label>
                <input type="text" value={brideMother} onChange={e => setBrideMother(e.target.value)} placeholder="Smt. Rekha Verma" className="w-full px-3 py-2.5 rounded-xl border border-[var(--border-gold)] bg-[var(--bg-primary)] text-xs text-[var(--text-primary)] focus:outline-none"/>
              </div>
            </div>

            <div>
              <label className="block text-xs font-semibold text-[var(--text-secondary)] mb-1">Bride Biography / Details</label>
              <textarea rows={2} value={brideBio} onChange={e => setBrideBio(e.target.value)} placeholder="A short note about the Bride..." className="w-full px-3 py-2 rounded-xl border border-[var(--border-gold)] bg-[var(--bg-primary)] text-xs text-[var(--text-primary)] focus:outline-none"/>
            </div>
          </LuxuryCard>
          ...
          <LuxuryCard className="space-y-4">
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
              <div>
                <CalendarPicker label="Main Wedding Date *" value={weddingDate} onChange={dateStr => setWeddingDate(dateStr)}/>
              </div>

              <div>
                <label className="block text-xs font-semibold text-[var(--text-secondary)] mb-1">Venue Name</label>
                <input type="text" value={venueName} onChange={e => setVenueName(e.target.value)} placeholder="e.g. The Royal Palace" className="w-full px-3 py-2.5 rounded-xl border border-[var(--border-gold)] bg-[var(--bg-primary)] text-xs text-[var(--text-primary)] focus:outline-none"/>
              </div>

              <div className="sm:col-span-2">
                <label className="block text-xs font-semibold text-[var(--text-secondary)] mb-1">Full Venue Address</label>
                <input type="text" value={venueAddress} onChange={e => setVenueAddress(e.target.value)} placeholder="Sector 62, Gurgaon, Delhi NCR" className="w-full px-3 py-2.5 rounded-xl border border-[var(--border-gold)] bg-[var(--bg-primary)] text-xs text-[var(--text-primary)] focus:outline-none"/>
              </div>

              <div className="sm:col-span-2">
                <label className="block text-xs font-semibold text-[var(--text-secondary)] mb-1">Google Maps Pin Link</label>
                <input type="url" value={googleMapsLink} onChange={e => setGoogleMapsLink(e.target.value)} placeholder="https://maps.google.com/?q=..." className="w-full px-3 py-2.5 rounded-xl border border-[var(--border-gold)] bg-[var(--bg-primary)] text-xs text-[var(--text-primary)] focus:outline-none"/>
              </div>
            </div>

            <div>
              <label className="block text-xs font-semibold text-[var(--text-secondary)] mb-1">Love Story Timeline Story</label>
              <textarea rows={4} value={loveStory} onChange={e => setLoveStory(e.target.value)} placeholder="How the Groom & Bride met and built their story..." className="w-full px-3 py-2 rounded-xl border border-[var(--border-gold)] bg-[var(--bg-primary)] text-xs text-[var(--text-primary)] focus:outline-none"/>
            </div>

            <button type="submit" disabled={savingWedding} className="w-full py-3.5 rounded-xl bg-gold-gradient text-white font-medium text-sm shadow-lg hover:opacity-95 transition-all flex items-center justify-center gap-2 cursor-pointer gold-shadow">
      ...
      {activeTab === 'members' && (<div className="space-y-6">
          ...
          {pendingMembers.length > 0 && (<LuxuryCard className="border-2 border-amber-500/40 bg-amber-500/5 space-y-4">
              ...
              <div className="space-y-3">
                {pendingMembers.map((p) => (<div key={p.id} className="p-4 rounded-xl bg-[var(--bg-card)] border border-[var(--border-gold)] flex items-center justify-between gap-4">
                    <div>
          ...
          <LuxuryCard className="space-y-4">
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 gap-3">
              {approvedMembers.map(m => (<div key={m.id} className="p-3 rounded-xl border border-[var(--border-subtle)] bg-[var(--bg-primary)] flex items-center gap-3">
                  <img src={m.avatar_url || `https://api.dicebear.com/7.x/avataaars/svg?seed=${m.full_name}`} alt={m.full_name} className="w-10 h-10 rounded-full border border-[var(--border-gold)]"/>
                  <div>
      ...
      {activeTab === 'events' && (<div className="space-y-6">
          ...
          <div className="space-y-3">
            {events.map(evt => (<LuxuryCard key={evt.id} className="flex items-center justify-between gap-4 p-4">
                <div>
                ...
                }} className="p-2 rounded-xl text-red-500 hover:bg-red-500/10 cursor-pointer" title="Delete Event">
          ...
          {showAddEventModal && (<div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-black/70 backdrop-blur-md animate-fade-in">
              <div className="relative w-full max-w-lg bg-[var(--bg-card)] border border-[var(--border-gold)] rounded-2xl p-6 shadow-2xl space-y-4">
                ...
                <form onSubmit={handleCreateEvent} className="space-y-3">
                  <div>
                    ...
                    <select value={eventType} onChange={e => setEventType(e.target.value)} className="w-full px-3 py-2 rounded-xl border border-[var(--border-gold)] bg-[var(--bg-primary)] text-xs text-[var(--text-primary)] focus:outline-none">
                  ...
                  {eventType === 'other' && (<div className="p-3 rounded-xl bg-[var(--gold-light)] border border-[var(--border-gold)] space-y-1">
                      ...
                      <input type="text" required value={customType} onChange={e => setCustomType(e.target.value)} placeholder="e.g. Tilak & Sagan" className="w-full px-3 py-2 rounded-xl border border-[var(--border-gold)] bg-[var(--bg-card)] text-xs text-[var(--text-primary)] focus:outline-none"/>
                  ...
                  <div>
                    ...
                    <input type="text" required value={eventTitle} onChange={e => setEventTitle(e.target.value)} placeholder="e.g. Auspicious Sagan Ceremony" className="w-full px-3 py-2 rounded-xl border border-[var(--border-gold)] bg-[var(--bg-primary)] text-xs text-[var(--text-primary)] focus:outline-none"/>
                  </div>

                  <div className="space-y-3">
                    <CalendarPicker label="Event Date *" value={eventDate} onChange={dateStr => setEventDate(dateStr)}/>
                    ...
                    <div className="grid grid-cols-2 gap-3">
                      <div>
                        ...
                        <div className="relative">
                          ...
                          <select value={startTime} onChange={e => setStartTime(e.target.value)} className="w-full pl-9 pr-3 py-2 rounded-xl border border-[var(--border-gold)] bg-[var(--bg-primary)] text-xs text-[var(--text-primary)] focus:outline-none focus:ring-1 focus:ring-[var(--gold-primary)] font-medium cursor-pointer">
                      ...
                      <div>
                        ...
                        <div className="relative">
                          <Clock className="absolute left-3 top-2.5 w-4 h-4 text-[var(--gold-dark)] pointer-events-none"/>
                          <select value={endTime} onChange={e => setEndTime(e.target.value)} className="w-full pl-9 pr-3 py-2 rounded-xl border border-[var(--border-gold)] bg-[var(--bg-primary)] text-xs text-[var(--text-primary)] focus:outline-none focus:ring-1 focus:ring-[var(--gold-primary)] font-medium cursor-pointer">
                  ...
                  <div>
                    <label className="block text-xs font-semibold text-[var(--text-secondary)] mb-1">Venue Name</label>
                    <input type="text" value={eventVenue} onChange={e => setEventVenue(e.target.value)} placeholder="Grand Ballroom" className="w-full px-3 py-2 rounded-xl border border-[var(--border-gold)] bg-[var(--bg-primary)] text-xs text-[var(--text-primary)] focus:outline-none"/>
                  </div>

                  <div>
                    <label className="block text-xs font-semibold text-[var(--text-secondary)] mb-1">Venue Address</label>
                    <input type="text" value={eventAddress} onChange={e => setEventAddress(e.target.value)} placeholder="Sector 62, Gurgaon" className="w-full px-3 py-2 rounded-xl border border-[var(--border-gold)] bg-[var(--bg-primary)] text-xs text-[var(--text-primary)] focus:outline-none"/>
                  </div>

                  <div>
                    <label className="block text-xs font-semibold text-[var(--text-secondary)] mb-1">Google Maps Link</label>
                    <input type="url" value={eventMapLink} onChange={e => setEventMapLink(e.target.value)} placeholder="https://maps.google.com/?q=..." className="w-full px-3 py-2 rounded-xl border border-[var(--border-gold)] bg-[var(--bg-primary)] text-xs text-[var(--text-primary)] focus:outline-none"/>
                  </div>

                  <button type="submit" className="w-full py-3 rounded-xl bg-gold-gradient text-white font-medium text-xs shadow hover:opacity-95 transition-all cursor-pointer mt-3">
      ...
      {activeTab === 'moderation' && (<div className="space-y-6">
          <LuxuryCard className="space-y-4">
            <h3 className="font-serif font-bold text-lg text-[var(--text-primary)]">
              Gallery Photo Moderation ({photos.length})
            </h3>
            <div className="grid grid-cols-2 sm:grid-cols-4 gap-3">
              {photos.map(p => (<div key={p.id} className="relative rounded-xl overflow-hidden border border-[var(--border-gold)] group">
          ...
          <LuxuryCard className="space-y-4">
            ...
            <div className="space-y-2">
              {wishes.map(w => (<div key={w.id} className="p-3 rounded-xl border border-[var(--border-subtle)] bg-[var(--bg-primary)] flex items-center justify-between text-xs">
                  <div>
