...
export const SuperAdminPortal = ({ onToast }) => {
    ...
    const handleCreateFamilyBySuper = (e) => {
        e.preventDefault();
    ...
    return (<div className="space-y-8 animate-fade-in">
      ...
      <div className="flex flex-col sm:flex-row items-start sm:items-center justify-between gap-4 border-b border-[var(--border-subtle)] pb-6">
        ...
        <button onClick={() => setShowCreateModal(true)} className="px-6 py-3 rounded-xl bg-purple-600 text-white font-medium text-xs shadow-lg hover:bg-purple-700 transition-all flex items-center gap-2 cursor-pointer shrink-0">
      ...
      <div className="space-y-4">
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
          {families.map((fam) => {
            ...
            return (<LuxuryCard key={fam.id} className={`space-y-4 border-2 ${isCurrent ? 'border-purple-500 bg-purple-500/5' : 'border-[var(--border-gold)]'}`}>
                <div className="flex items-start justify-between">
                  ...
                  {isCurrent ? (<span className="px-3 py-1 rounded-full bg-purple-600 text-white text-[10px] font-bold uppercase tracking-wider">
                      Active Portal
                    </span>) : (<button onClick={() => {
                        switchFamily(fam.id);
      ...
      {showCreateModal && (<div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-black/70 backdrop-blur-md animate-fade-in">
            ...
            <form onSubmit={handleCreateFamilyBySuper} className="space-y-3">
              <div className="grid grid-cols-2 gap-3">
                <div>
                  ...
                  <input type="text" required value={newFamilyName} onChange={e => setNewFamilyName(e.target.value)} placeholder="e.g. Roy Family" className="w-full px-3 py-2 rounded-xl border border-[var(--border-gold)] bg-[var(--bg-primary)] text-xs text-[var(--text-primary)] focus:outline-none"/>
                </div>

                <div>
                  <label className="block text-xs font-semibold text-[var(--text-secondary)] mb-1">Shared Passkey *</label>
                  <input type="text" required value={newFamilyPass} onChange={e => setNewFamilyPass(e.target.value)} placeholder="roy123" className="w-full px-3 py-2 rounded-xl border border-[var(--border-gold)] bg-[var(--bg-primary)] text-xs text-[var(--text-primary)] focus:outline-none"/>
                </div>
              </div>
              ...
              <div className="pt-2 border-t border-[var(--border-subtle)]">
                ...
                <div className="space-y-2">
                  <input type="text" value={adminName} onChange={e => setAdminName(e.target.value)} placeholder="Admin Full Name (e.g. Sneh Roy)" className="w-full px-3 py-2 rounded-xl border border-[var(--border-gold)] bg-[var(--bg-primary)] text-xs text-[var(--text-primary)] focus:outline-none"/>
                  <input type="email" value={adminEmail} onChange={e => setAdminEmail(e.target.value)} placeholder="Admin Email (sneh.roy@gmail.com)" className="w-full px-3 py-2 rounded-xl border border-[var(--border-gold)] bg-[var(--bg-primary)] text-xs text-[var(--text-primary)] focus:outline-none"/>
                  <input type="password" value={adminPass} onChange={e => setAdminPass(e.target.value)} placeholder="Admin Password (pass123)" className="w-full px-3 py-2 rounded-xl border border-[var(--border-gold)] bg-[var(--bg-primary)] text-xs text-[var(--text-primary)] focus:outline-none"/>
                </div>
              </div>

              <button type="submit" className="w-full py-3 rounded-xl bg-purple-600 text-white font-medium text-xs shadow hover:bg-purple-700 transition-all cursor-pointer mt-3">
