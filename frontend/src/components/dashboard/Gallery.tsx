...
interface GalleryProps {
  photos: GalleryPhoto[];
  // LocalStorage mode: receives base64 {url, caption}
  onBatchUploadPhotos: (items: Array<{ url: string; caption: string }>) => { uploaded: number; skipped: number };
  // Supabase mode: receives actual File objects (preferred)
  onBatchUploadFiles?: (items: Array<{ file: File; caption: string }>) => Promise<{ uploaded: number; skipped: number }>;
...
export const Gallery: React.FC<GalleryProps> = ({
  ...
  // Filter & Sort State
  const [filterMode, setFilterMode] = useState<'latest' | 'liked' | 'my'>('latest');
  const [activePhoto, setActivePhoto] = useState<GalleryPhoto | null>(null);

  // Upload Modal State
  const [showUploadModal, setShowUploadModal] = useState(false);
  // Each item stores both a preview URL (blob/base64) and optionally the original File
  const [selectedPhotos, setSelectedPhotos] = useState<Array<{ id: string; previewUrl: string; caption: string; file?: File }>>([]);
  ...
  const handleDrop = async (e: React.DragEvent<HTMLDivElement>) => {
    ...
    const validFiles = files.filter(f => f.type.startsWith('image/'));
    if (validFiles.length === 0) { onToast('Please drop valid image files.', 'error'); return; }

    const currentTotal = userPhotoCount + selectedPhotos.length;
    const availableSlots = MAX_PHOTOS - currentTotal;
    if (availableSlots <= 0) { onToast(`Limit reached! Maximum ${MAX_PHOTOS} photos allowed per member.`, 'error'); return; }

    const filesToProcess = validFiles.slice(0, availableSlots);
    if (validFiles.length > availableSlots) onToast(`Only ${availableSlots} slot(s) remaining. Added first ${availableSlots}.`, 'info');
  ...
  const closeUploadModal = () => {
    ...
    selectedPhotos.forEach(p => { if (p.previewUrl?.startsWith('blob:')) URL.revokeObjectURL(p.previewUrl); });
    setSelectedPhotos([]);
  ...
  const handleUploadSubmit = async (e: React.FormEvent) => {
    e.preventDefault();
    if (selectedPhotos.length === 0) { onToast('Please select at least one photo file first!', 'error'); return; }
    ...
    <div className="space-y-8 animate-fade-in">
      ...
      <div className="flex flex-col sm:flex-row items-start sm:items-center justify-between gap-4 border-b border-[var(--border-subtle)] pb-6">
        ...
        <div className="flex items-center gap-3 w-full sm:w-auto">
          ...
          <button
              onClick={() => {
                if (userPhotoCount >= MAX_PHOTOS) { onToast(`10 Photo limit reached for your member account!`, 'error'); return; }
                setShowUploadModal(true);
              }}
              disabled={!isApproved}
              className="w-full sm:w-auto px-6 py-3 rounded-xl bg-gold-gradient text-white font-medium text-xs shadow-md hover:opacity-95 transition-all flex items-center justify-center gap-2 cursor-pointer gold-shadow"
            >
      ...
      {filteredPhotos.length === 0 ? (
        <EmptyState
          ...
          description="Be the first family member to upload a wedding photo!"
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-6">
          {filteredPhotos.map((photo) => {
            ...
            return (
              <LuxuryCard key={photo.id} className="p-0 overflow-hidden group border border-[var(--border-gold)] flex flex-col justify-between">
                <div
                  onClick={() => setActivePhoto(photo)}
                  className="relative h-64 overflow-hidden cursor-pointer bg-stone-900"
                >
                  <img
                    src={photo.photo_url}
                  ...
                  {(isOwner || isFamilyAdmin) && onDeletePhoto && (
                    <button
                      onClick={(e) => {
                        e.stopPropagation();
                      ...
                      className="absolute top-3 right-3 p-2 rounded-xl bg-black/60 backdrop-blur-md text-red-400 hover:text-red-300 border border-white/20 transition-all cursor-pointer opacity-0 group-hover:opacity-100"
                      title="Delete photo"
                    >
                ...
                <div className="p-4 flex items-center justify-between border-t border-[var(--border-subtle)] text-xs">
                  ...
                  <button
                    onClick={() => setActivePhoto(photo)}
                    className="flex items-center gap-1.5 text-[var(--text-muted)] hover:text-[var(--text-primary)] font-semibold cursor-pointer"
                  >
            ...
            <form onSubmit={handleUploadSubmit} className="space-y-4 overflow-y-auto flex-1 pr-1">
              <div>
                ...
                <div
                  ...
                  onDrop={handleDrop}
                  className="p-6 border-2 border-dashed border-[var(--border-gold)] rounded-2xl bg-[var(--bg-accent)] text-center cursor-pointer hover:bg-[var(--gold-light)]/40 transition-all space-y-2 group gold-shadow"
                >
                ...
                <div className="space-y-3">
                  <div className="flex items-center justify-between text-xs font-bold text-[var(--text-primary)]">
                    ...
                    <button
                      ...
                      className="text-xs text-red-500 hover:underline cursor-pointer font-normal"
                    >
                      Clear Selection
                    </button>
                  </div>
                  ...
                  <div className="grid grid-cols-2 sm:grid-cols-3 gap-3 max-h-72 overflow-y-auto p-1">
                    {selectedPhotos.map((item, index) => (
                      <div key={item.id} className="relative group bg-[var(--bg-primary)] border border-[var(--border-gold)] rounded-xl p-2 space-y-1.5 shadow-sm">
                        <div className="relative">
                          <img
                            src={item.previewUrl}
                            alt={`Upload ${index + 1}`}
                            className="w-full h-28 object-cover rounded-lg border border-[var(--border-subtle)]"
                          />
                          <button
                            type="button"
                        ...
                        <input
                          ...
                          className="w-full px-2 py-1 text-[10px] rounded-md border border-[var(--border-subtle)] bg-[var(--bg-card)] text-[var(--text-primary)] focus:outline-none focus:ring-1 focus:ring-[var(--gold-primary)]"
                        />
                      </div>
                    ))}
                  </div>
              ...
              <button
                ...
                className="w-full py-3 rounded-xl bg-gold-gradient text-white font-medium text-xs shadow-md hover:opacity-95 transition-all cursor-pointer shrink-0"
              >
                {uploading
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-black/85 backdrop-blur-md animate-fade-in overflow-y-auto">
          <div className="relative w-full max-w-4xl bg-[var(--bg-card)] border border-[var(--border-gold)] rounded-2xl overflow-hidden shadow-2xl my-6 grid grid-cols-1 md:grid-cols-2">
            <button
              ...
              className="absolute top-4 right-4 z-10 p-2 rounded-full bg-black/60 text-white hover:bg-black cursor-pointer"
            >
              <X className="w-5 h-5" />
            </button>
            ...
            <div className="p-6 flex flex-col justify-between space-y-4 max-h-[70vh] overflow-y-auto">
              <div className="space-y-3">
                ...
                <div className="space-y-2 max-h-60 overflow-y-auto pr-1">
                  ...
                  {!activePhoto.comments || activePhoto.comments.length === 0 ? (
                    <p className="text-xs text-[var(--text-muted)] italic">No comments yet. Write the first one!</p>
                  ) : (
                    activePhoto.comments.map(c => (
                      <div key={c.id} className="p-2.5 rounded-xl bg-[var(--bg-accent)] text-xs space-y-1">
              ...
              <form onSubmit={handleSendComment} className="flex gap-2 pt-3 border-t border-[var(--border-subtle)]">
                <input
                  ...
                  className="flex-1 px-3 py-2 rounded-xl border border-[var(--border-gold)] bg-[var(--bg-primary)] text-xs text-[var(--text-primary)] focus:outline-none"
                />
                <button
                  type="submit"
                  className="p-2.5 rounded-xl bg-gold-gradient text-white hover:opacity-95 transition-all cursor-pointer"
                >
