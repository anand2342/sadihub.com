...
export const DigitalInvitation: React.FC<DigitalInvitationProps> = ({ wedding, events = [], onToast }) => {
  if (!wedding) {
    return (
      <EmptyState
        title="Invitation Card Not Ready"
        description="Your Family Admin will publish the royal invitation card shortly."
      />
    );
  }
  ...
  return (
    <div className="space-y-8 animate-fade-in max-w-4xl mx-auto">
      ...
      <div className="flex items-center justify-end gap-3 print:hidden">
        <button
          onClick={handleShare}
          className="px-4 py-2 rounded-xl border border-[var(--border-gold)] bg-[var(--bg-card)] text-[var(--text-primary)] font-medium text-xs hover:bg-[var(--bg-accent)] transition-all flex items-center gap-1.5 cursor-pointer"
        >
        ...
        <button
          onClick={handlePrint}
          className="px-5 py-2 rounded-xl bg-gold-gradient text-white font-medium text-xs shadow hover:opacity-95 transition-all flex items-center gap-1.5 cursor-pointer"
        >
      ...
      <div className="printable-invitation-card relative rounded-3xl border-4 border-[var(--border-gold)] p-8 sm:p-14 bg-gradient-to-b from-[var(--bg-card)] via-[var(--bg-surface)] to-[var(--bg-card)] gold-shadow text-center space-y-8 print:bg-white print:border-[#B8860B] print:p-5 print:shadow-none print:space-y-3 print:rounded-2xl">
          ...
          <div className="flex items-center justify-center gap-4 sm:gap-8 my-2 print:my-1 print:gap-4">
            ...
            <div className="relative p-1 rounded-full border border-dashed border-[var(--border-gold)] print:border-[#B8860B]">
              <div className="w-24 h-24 sm:w-32 sm:h-32 print:w-20 print:h-20 rounded-full overflow-hidden border-2 border-[var(--border-gold)] print:border-[#B8860B] bg-white/40 print:bg-white flex items-center justify-center">
                <img
                  src={wedding.groom_image || 'https://images.unsplash.com/photo-1507003211169-0a1dd7228f2d?auto=format&fit=crop&w=800&q=80'}
                  alt={wedding.groom_name}
                  className="w-full h-full object-cover portrait-vignette-blend"
                />
              </div>
            ...
            <div className="relative p-1 rounded-full border border-dashed border-[var(--border-gold)] print:border-[#B8860B]">
              <div className="w-24 h-24 sm:w-32 sm:h-32 print:w-20 print:h-20 rounded-full overflow-hidden border-2 border-[var(--border-gold)] print:border-[#B8860B] bg-white/40 print:bg-white flex items-center justify-center">
                <img
                  src={wedding.bride_image || 'https://images.unsplash.com/photo-1534528741775-53994a69daeb?auto=format&fit=crop&w=800&q=80'}
        ...
        {events && events.length > 0 && (
          <div className="py-5 space-y-4 border-t border-[var(--border-gold)] print:border-[#B8860B] print:py-2 print:space-y-2">
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-3 text-left print:gap-2">
              {events.map((evt) => {
                ...
                return (
                  <div
                    key={evt.id}
                    className="p-3.5 rounded-2xl bg-[var(--bg-accent)]/60 border border-[var(--border-gold)] print:bg-[#FFFDF8] print:border-[#B8860B] space-y-1.5 shadow-sm print:shadow-none flex flex-col justify-between print:p-2 print:rounded-xl"
                  >
                    <div className="space-y-1 print:space-y-0.5">
                      <div className="flex items-center justify-between gap-2">
                        ...
                        {evt.start_time && (
                          <span className="text-[10px] font-mono text-[var(--text-muted)] print:text-[#4A4A4A] print:text-[9px] flex items-center gap-1">
                      ...
                      {evt.description && (
                        <p className="text-[11px] text-[var(--text-secondary)] print:text-[#333333] print:text-[9px] line-clamp-1 italic font-serif leading-snug">
                          "{evt.description}"
                        </p>
                      )}
                    </div>
                    ...
                    <div className="pt-2 border-t border-[var(--border-subtle)] print:border-amber-200 text-[11px] text-[var(--text-secondary)] print:text-[#222222] print:text-[9px] print:pt-1 space-y-0.5 font-sans">
                      ...
                      {evt.venue_name && (
                        <p className="font-medium line-clamp-1 text-[var(--text-primary)] print:text-[#1A1A1A] flex items-center gap-1">
