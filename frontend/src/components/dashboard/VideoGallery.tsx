...
export const VideoGallery: React.FC<VideoGalleryProps> = ({ videos, onDeleteVideo, onToast }) => {
  ...
  if (!videos || videos.length === 0) {
    return (
      <EmptyState
        icon={Video}
        title="No Wedding Videos Published"
        description="Pre-wedding film teasers, Sangeet performances, and live streams will be uploaded here by your Family Admin."
      />
    );
  }
  ...
  return (
    <div className="space-y-8 animate-fade-in">
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 gap-8">
        {videos.map((vid) => (
          <LuxuryCard key={vid.id} className="p-0 overflow-hidden space-y-4 flex flex-col justify-between">
              ...
              {vid.video_url.includes('youtube.com') || vid.video_url.includes('youtu.be') ? (
                ...
                <div className="w-full h-full flex flex-col items-center justify-center p-6 text-center text-white bg-gradient-to-br from-stone-900 to-black">
                  ...
                  <a
                    ...
                    target="_blank"
                    rel="noreferrer"
                    className="mt-3 px-4 py-2 rounded-xl bg-gold-gradient text-xs text-white font-semibold"
                  >
                    Open Video Stream
                  </a>
                </div>
            ...
            <div className="p-5 flex items-center justify-between">
              ...
              {isFamilyAdmin && onDeleteVideo && (
                <button
                  onClick={() => {
                    onDeleteVideo(vid.id);
                    onToast('Video deleted.', 'info');
                  }}
                  className="text-red-500/60 hover:text-red-500 p-2 rounded-xl cursor-pointer"
                  title="Delete Video"
                >
