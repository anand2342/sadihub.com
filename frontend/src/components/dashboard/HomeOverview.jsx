...
export const HomeOverview = ({ wedding, events, onNavigate }) => {
    ...
    return (<div className="space-y-8 animate-fade-in">
      ...
      <div className="relative rounded-3xl overflow-hidden gold-border gold-shadow p-6 sm:p-10 bg-gradient-to-r from-stone-900/90 via-stone-900/80 to-stone-900/40 text-white min-h-[300px] flex flex-col justify-between">
        {wedding?.cover_image && (<img src={wedding.cover_image} alt="Wedding Cover" className="absolute inset-0 w-full h-full object-cover mix-blend-overlay opacity-50 pointer-events-none"/>)}
      ...
      <div className="grid grid-cols-1 xl:grid-cols-2 gap-6">
        ...
        <LuxuryCard className="flex flex-row items-center gap-3.5 sm:gap-5 p-4 sm:p-5 overflow-hidden">
          <img src={wedding?.groom_image || 'https://images.unsplash.com/photo-1507003211169-0a1dd7228f2d?auto=format&fit=crop&w=300&q=80'} alt="Groom" className="w-20 h-20 sm:w-28 sm:h-28 rounded-2xl object-cover border-2 border-[var(--border-gold)] shrink-0 shadow-md"/>
          <div className="space-y-1 sm:space-y-1.5 min-w-0 flex-1">
            ...
            <button onClick={() => onNavigate('couple')} className="text-xs font-semibold text-[var(--gold-dark)] dark:text-[var(--gold-primary)] hover:underline inline-flex items-center gap-1 pt-0.5 cursor-pointer whitespace-nowrap">
        ...
        {/* BRIDE CARD SECOND */}
        <LuxuryCard className="flex flex-row items-center gap-3.5 sm:gap-5 p-4 sm:p-5 overflow-hidden">
          <img src={wedding?.bride_image || 'https://images.unsplash.com/photo-1534528741775-53994a69daeb?auto=format&fit=crop&w=300&q=80'} alt="Bride" className="w-20 h-20 sm:w-28 sm:h-28 rounded-2xl object-cover border-2 border-[var(--border-gold)] shrink-0 shadow-md"/>
          <div className="space-y-1 sm:space-y-1.5 min-w-0 flex-1">
            <div>
              <span className="inline-block whitespace-nowrap text-[10px] font-bold tracking-widest uppercase text-[var(--gold-dark)] dark:text-[var(--gold-primary)] px-2.5 py-0.5 rounded-full border border-[var(--border-gold)] bg-[var(--bg-accent)]">
                The Bride
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
        <LuxuryCard className="lg:col-span-2 space-y-4">
          <div className="flex items-center justify-between">
            ...
            <button onClick={() => onNavigate('events')} className="text-xs text-[var(--gold-dark)] dark:text-[var(--gold-primary)] font-semibold hover:underline cursor-pointer">
              View All Events ({events.length})
            </button>
          </div>

          {nextEvent ? (<div className="p-4 rounded-2xl border border-[var(--border-gold)] bg-[var(--bg-accent)] flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4">
              <div className="space-y-1">
                ...
                <div className="flex items-center gap-4 text-xs text-[var(--text-muted)] pt-2">
                  ...
                  {nextEvent.start_time && (<span className="flex items-center gap-1">
              ...
              <button onClick={() => onNavigate('events')} className="w-full sm:w-auto px-5 py-2.5 rounded-xl bg-gold-gradient text-white text-xs font-semibold shadow cursor-pointer shrink-0">
                Event Details
              </button>
            </div>) : (<p className="text-xs text-[var(--text-muted)] italic">No events scheduled yet.</p>)}
        </LuxuryCard>
        ...
        <LuxuryCard className="space-y-3 flex flex-col justify-between">
          <div>
            ...
            <div className="space-y-2">
              <button onClick={() => onNavigate('gallery')} className="w-full p-3 rounded-xl border border-[var(--border-subtle)] bg-[var(--bg-primary)] hover:border-[var(--border-gold)] transition-all flex items-center justify-between text-xs font-medium cursor-pointer">
                <div className="flex items-center gap-2">
              ...
              <button onClick={() => onNavigate('wishes')} className="w-full p-3 rounded-xl border border-[var(--border-subtle)] bg-[var(--bg-primary)] hover:border-[var(--border-gold)] transition-all flex items-center justify-between text-xs font-medium cursor-pointer">
                <div className="flex items-center gap-2">
              ...
              <button onClick={() => onNavigate('invitation')} className="w-full p-3 rounded-xl border border-[var(--border-subtle)] bg-[var(--bg-primary)] hover:border-[var(--border-gold)] transition-all flex items-center justify-between text-xs font-medium cursor-pointer">
                <div className="flex items-center gap-2">
