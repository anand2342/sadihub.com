...
export const CoupleDetails: React.FC<CoupleDetailsProps> = ({ wedding }) => {
  if (!wedding) {
    return (
      <EmptyState
        title="No Couple Details Found"
        description="Your Family Admin hasn't added the Groom & Bride details yet."
      />
    );
  }
              ...
              <div>
                ...
                <div className="flex flex-wrap gap-4 text-xs text-[var(--text-muted)] mt-2 font-medium">
                  {wedding.groom_father && (
                    <span>Son of: <strong className="text-[var(--text-primary)]">{wedding.groom_father}</strong></span>
                  )}
                  {wedding.groom_mother && (
                    <span>& <strong className="text-[var(--text-primary)]">{wedding.groom_mother}</strong></span>
                  )}
                </div>
              </div>
              ...
              <div>
                ...
                <div className="flex flex-wrap gap-4 text-xs text-[var(--text-muted)] mt-2 font-medium">
                  {wedding.bride_father && (
                    <span>Daughter of: <strong className="text-[var(--text-primary)]">{wedding.bride_father}</strong></span>
                  )}
                  {wedding.bride_mother && (
                    <span>& <strong className="text-[var(--text-primary)]">{wedding.bride_mother}</strong></span>
                  )}
                </div>
              </div>
