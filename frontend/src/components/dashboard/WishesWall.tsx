...
import type { Wish, RelationType } from '../../types';
...
interface WishesWallProps {
  wishes: Wish[];
  onAddWish: (wishData: { sender_name: string; relation: RelationType; message: string }) => void;
...
export const WishesWall: React.FC<WishesWallProps> = ({
  ...
  const [relation, setRelation] = useState<RelationType>(session?.profile?.relation || 'Cousin');
    ...
    <div className="space-y-8 animate-fade-in">
        ...
        <form onSubmit={handleSubmit} className="space-y-4">
          ...
          <div>
            ...
            <textarea
              ...
              placeholder="May your marriage be blessed with endless happiness, health, and laughter..."
              className="w-full px-3 py-2 rounded-xl border border-[var(--border-gold)] bg-[var(--bg-primary)] text-xs text-[var(--text-primary)] focus:outline-none"
            />
          </div>
          ...
          <button
            type="submit"
            disabled={submitting}
            className="w-full py-3 rounded-xl bg-gold-gradient text-white font-medium text-xs shadow-md hover:opacity-95 transition-all flex items-center justify-center gap-2 cursor-pointer"
          >
      ...
      {wishes.length === 0 ? (
        <EmptyState
          ...
          title="No Wishes Posted Yet"
          description="Be the first family member to write a blessing on the wall!"
        />
      ) : (
        <div className="grid grid-cols-1 md:grid-cols-2 gap-6 max-w-5xl mx-auto">
          {wishes.map((w) => (
            <LuxuryCard key={w.id} className="relative flex flex-col justify-between h-full space-y-4">
              <div className="space-y-3">
                <div className="flex items-center justify-between">
                  ...
                  {isFamilyAdmin && onDeleteWish && (
                    <button
                      onClick={() => {
                        onDeleteWish(w.id);
                        onToast('Wish removed by admin moderation.', 'info');
                      }}
                      className="text-red-500/60 hover:text-red-500 p-1 rounded-lg cursor-pointer"
                      title="Moderate / Delete wish"
                    >
