...
export const MapLocation = ({ wedding }) => {
    if (!wedding) {
        return (<EmptyState icon={MapPin} title="Location Details Pending" description="Your Family Admin will provide venue addresses and interactive map links soon."/>);
    }
    ...
    return (<div className="space-y-8 animate-fade-in max-w-4xl mx-auto">
      ...
      <LuxuryCard className="space-y-6 border-2 border-[var(--border-gold)] gold-shadow">
        ...
        <div className="p-6 rounded-2xl bg-[var(--bg-accent)] border border-[var(--border-gold)] text-center space-y-4">
          ...
          <a href={mapLink} target="_blank" rel="noreferrer" className="inline-flex items-center gap-2 px-8 py-3 rounded-xl bg-gold-gradient text-white font-medium text-xs shadow-md hover:opacity-95 transition-all cursor-pointer gold-shadow">
