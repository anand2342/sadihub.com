...
export const EventSchedule: React.FC<EventScheduleProps> = ({ events }) => {
  if (!events || events.length === 0) {
    return (
      <EmptyState
        icon={Calendar}
        title="No Wedding Events Scheduled"
        description="Event schedules (Haldi, Mehendi, Sangeet, Tilak, Wedding, Reception) will be published here by your Family Admin."
      />
    );
  }
    ...
    <div className="space-y-8 animate-fade-in">
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
        {events.map((evt) => {
          ...
          return (
            <LuxuryCard key={evt.id} className="flex flex-col justify-between h-full space-y-4">
              ...
              <div className="pt-4 border-t border-[var(--border-subtle)] space-y-2 text-xs">
                ...
                {evt.google_maps_link && (
                  <a
                    ...
                    target="_blank"
                    rel="noreferrer"
                    className="inline-flex items-center gap-1.5 text-xs font-semibold text-[var(--gold-dark)] dark:text-[var(--gold-primary)] hover:underline pt-2"
                  >
