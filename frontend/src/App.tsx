...
import {
  dbGetWedding, dbUpsertWedding,
  dbGetEvents, dbInsertEvent, dbDeleteEvent,
  dbGetFamilyProfiles, dbUpdateMemberStatus, dbDeleteProfile,
  dbGetPhotos, dbUploadPhotoFile, dbCountUserPhotos, dbToggleLike, dbAddComment, dbDeletePhoto,
  dbGetWishes, dbInsertWish, dbDeleteWish,
  dbGetVideos, dbDeleteVideo,
} from './lib/supabaseApi';
import type { Wedding, EventItem, Wish, GalleryPhoto, Profile } from './types';
...
const MainAppContent: React.FC = () => {
  ...
  const handleUpdateWedding = async (updated: Partial<Wedding>) => {
  ...
  const handleAddEvent = async (eventData: Omit<EventItem, 'id' | 'created_at'>) => {
  ...
  const handleApproveMember = async (profileId: string) => {
    ...
    if (idx >= 0) { store.profiles[idx].status = 'approved'; saveStore(store); refreshLocalData(); }
  };
  ...
  const handleBatchUploadFiles = async (items: Array<{ file: File; caption: string }>): Promise<{ uploaded: number; skipped: number }> => {
    if (!items.length) return { uploaded: 0, skipped: 0 };
  ...
  const handleBatchUploadPhotos = (items: Array<{ url: string; caption: string }>): { uploaded: number; skipped: number } => {
    if (!items || items.length === 0) return { uploaded: 0, skipped: 0 };
    const store = getStore();
    let existingCount = store.photos.filter(p => p.family_id === currentFamilyIdResolved && p.user_id === userId).length;
    const MAX_PHOTOS = 10;
    let uploaded = 0, skipped = 0;
    items.forEach((item, i) => {
      if (existingCount >= MAX_PHOTOS) { skipped++; return; }
    ...
    if (uploaded > 0) { saveStore(store); refreshLocalData(); }
    return { uploaded, skipped };
  };
  ...
  const handleToggleLike = async (photoId: string) => {
    ...
    if (idx >= 0) {
      ...
      if (p.user_has_liked) { p.user_has_liked = false; p.likes_count = Math.max(0, (p.likes_count || 1) - 1); }
      else { p.user_has_liked = true; p.likes_count = (p.likes_count || 0) + 1; }
      saveStore(store);
      refreshLocalData();
    }
  };
  ...
  const handleAddComment = async (photoId: string, comment: string) => {
    ...
    const idx = store.photos.findIndex(p => p.id === photoId);
    if (idx >= 0) {
      if (!store.photos[idx].comments) store.photos[idx].comments = [];
  ...
  const handleDeletePhoto = async (photoId: string) => {
    if (isSupabaseConfigured) {
      ...
      await dbDeletePhoto(photoId, (photo as any)?.storage_path);
  ...
  const handleAddWish = async (wishData: { sender_name: string; relation: any; message: string }) => {
    if (isSupabaseConfigured && currentFamilyId) {
  ...
  const handleOpenAuthTab = (tab: 'signin' | 'join' | 'create') => {
  ...
  if (!session) {
    return (
      <div className="min-h-screen bg-[var(--bg-primary)] text-[var(--text-primary)] transition-colors flex flex-col justify-between">
  ...
  return (
    <div className="min-h-screen bg-[var(--bg-primary)] text-[var(--text-primary)] transition-colors flex flex-col">
      <Header
        ...
        onOpenMobileMenu={() => setIsOpenMobileSidebar(prev => !prev)}
        onOpenAuth={() => handleOpenAuthTab('signin')}
      />
        ...
        <main className="flex-1 p-4 sm:p-6 lg:p-8 min-w-0">
          ...
          {activeTab === 'gallery' && (
            <Gallery
              ...
              onBatchUploadPhotos={handleBatchUploadPhotos}
              onBatchUploadFiles={isSupabaseConfigured ? handleBatchUploadFiles : undefined}
          ...
          {activeTab === 'wishes'     && (
            <WishesWall wishes={resolvedWishes} onAddWish={handleAddWish} onDeleteWish={handleDeleteWish} onToast={addToast} />
          )}
          {activeTab === 'videos'     && (
            <VideoGallery videos={resolvedVideos} onDeleteVideo={handleDeleteVideo} onToast={addToast} />
          )}
