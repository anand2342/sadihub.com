...
export const ProfileSetupModal: React.FC<ProfileSetupModalProps> = ({ onToast }) => {
    ...
    <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-black/70 backdrop-blur-md animate-fade-in">
      <div className="relative w-full max-w-md bg-[var(--bg-card)] border border-[var(--border-gold)] rounded-2xl p-6 sm:p-8 shadow-2xl text-center space-y-6">
        ...
        <form onSubmit={handleUpdate} className="text-left space-y-3 pt-2 border-t border-[var(--border-subtle)]">
          ...
          <div>
            ...
            <input
              ...
              className="w-full px-3 py-2 rounded-xl border border-[var(--border-subtle)] bg-[var(--bg-primary)]/50 text-xs text-[var(--text-muted)] cursor-not-allowed"
            />
          </div>
          ...
          <div className="space-y-2">
            <button
              type="submit"
              disabled={saving}
              className="w-full py-2.5 rounded-xl border border-[var(--border-gold)] bg-[var(--bg-accent)] text-[var(--text-primary)] font-medium text-xs hover:bg-[var(--gold-light)] transition-all cursor-pointer"
            >
            ...
            <button
              ...
              disabled={checking}
              className="w-full py-3 rounded-xl bg-gold-gradient text-white font-bold text-xs shadow-md hover:opacity-95 transition-all cursor-pointer gold-shadow flex items-center justify-center gap-2"
            >
        ...
        <div className="pt-4 flex items-center justify-between border-t border-[var(--border-subtle)] text-xs">
          ...
          <button
            onClick={logout}
            className="inline-flex items-center gap-1 text-amber-600 hover:text-amber-700 font-medium cursor-pointer"
          >
