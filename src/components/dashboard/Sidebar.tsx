...
export const Sidebar: React.FC<SidebarProps> = ({
  ...
  return (
    <>
      ...
      <aside
        className={cn(
          'fixed md:sticky top-0 md:top-20 left-0 z-50 md:z-30 h-screen md:h-[calc(100vh-5rem)] bg-[var(--bg-surface)] flex flex-col justify-between transition-all duration-300 ease-in-out overflow-y-auto print:hidden shrink-0',
          isOpenMobile 
      ...
      >
        <div className="space-y-6">
          ...
          <div className="flex items-center justify-between pb-4 border-b border-[var(--border-subtle)]">
            ...
            <button
              onClick={onCloseMobile}
              className="p-1.5 rounded-lg text-[var(--text-muted)] hover:text-[var(--text-primary)] hover:bg-[var(--bg-accent)] transition-colors cursor-pointer"
              title="Hide Sidebar"
            >
              <X className="w-5 h-5" />
            </button>
            ...
            {navItems.map(item => {
              ...
              return (
                <button
                  ...
                  className={cn(
                    'w-full flex items-center justify-between px-3 py-2.5 rounded-xl font-medium text-xs sm:text-sm transition-all cursor-pointer group',
                    isActive
          ...
          {(isFamilyAdmin || isSuperAdmin) && (
            <div className="pt-4 border-t border-[var(--border-subtle)] space-y-1">
              ...
              {(isFamilyAdmin || isSuperAdmin) && (
                <button
                  onClick={() => handleSelectTab('family-admin')}
                  className={cn(
                    'w-full flex items-center justify-between px-3 py-2.5 rounded-xl font-medium text-xs sm:text-sm transition-all cursor-pointer group',
