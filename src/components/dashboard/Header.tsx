...
export const Header: React.FC<HeaderProps> = ({ wedding, onOpenMobileMenu, onOpenAuth }) => {
  ...
  return (
    <header className="sticky top-0 z-40 bg-[var(--bg-surface)]/95 backdrop-blur-md border-b border-[var(--border-gold)] transition-colors print:hidden">
      ...
      <div className="w-full px-4 sm:px-6 lg:px-8 2xl:px-12 h-20 flex items-center justify-between gap-4">
        ...
        <div className="flex items-center gap-3 sm:gap-4">
          <button
            ...
            className="p-2 rounded-xl text-[var(--text-primary)] hover:bg-[var(--bg-accent)] border border-[var(--border-gold)] cursor-pointer gold-shadow"
            aria-label="Toggle menu"
            title="Toggle Sidebar Menu (Open / Hide)"
          >
        ...
        <div className="flex items-center gap-2 sm:gap-4">
          {session && (
            <div className="hidden lg:flex flex-col text-right">
              ...
              <span className="text-[10px] text-[var(--text-muted)] uppercase tracking-wider flex items-center justify-end gap-1">
                ...
                {!isSuperAdmin && isFamilyAdmin && (
                  <span className="text-[var(--gold-dark)] dark:text-[var(--gold-primary)] font-bold flex items-center gap-0.5">
                    <Shield className="w-3 h-3" /> Family Admin
                  </span>
                )}
                {!isSuperAdmin && !isFamilyAdmin && (
                  <span>{session.profile?.relation || 'Family Member'}</span>
                )}
              </span>
            </div>
          )}
          ...
          <button
            onClick={toggleTheme}
            className="px-3 py-2 rounded-xl border border-[var(--border-gold)] bg-[var(--bg-card)] text-[var(--text-primary)] hover:bg-[var(--bg-accent)] transition-all cursor-pointer gold-shadow flex items-center gap-1.5 text-xs font-semibold"
          ...
          {session ? (
            <button
              onClick={logout}
              className="p-2.5 sm:px-4 sm:py-2.5 rounded-xl border border-red-500/30 bg-red-500/10 text-red-600 dark:text-red-400 hover:bg-red-500/20 font-medium text-xs flex items-center gap-1.5 transition-colors cursor-pointer"
              title="Sign Out"
            >
            ...
            <button
              onClick={onOpenAuth}
              className="px-5 py-2.5 rounded-xl bg-gold-gradient text-white font-medium text-xs shadow-md hover:opacity-95 transition-all cursor-pointer"
            >
              Sign In
            </button>
          )}
