    ...
    <div className="space-y-8 animate-fade-in">
        ...
        <LuxuryCard className="border-2 border-amber-500/40 bg-amber-500/5 space-y-4">
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
            {pendingMembers.map((member) => (
              <div
                key={member.id}
                className="p-4 rounded-xl bg-[var(--bg-card)] border border-[var(--border-gold)] flex items-center justify-between gap-4"
              >
                <div className="flex items-center gap-3">
                  <img
                    src={member.avatar_url || `https://api.dicebear.com/7.x/avataaars/svg?seed=${member.full_name}`}
                    alt={member.full_name}
                    className="w-12 h-12 rounded-full border border-[var(--border-gold)] object-cover"
                  />
                  <div>
                    ...
                    {member.mobile_number && (
                      <p className="text-[10px] text-[var(--text-muted)]">{member.mobile_number}</p>
                    )}
                  </div>
                </div>
                ...
                <div className="flex items-center gap-2 shrink-0">
                  <button
                    ...
                    className="p-2 rounded-xl bg-emerald-600 text-white text-xs font-semibold hover:bg-emerald-700 transition-all flex items-center gap-1 cursor-pointer"
                  ...
                  <button
                    ...
                    className="p-2 rounded-xl border border-red-500/40 text-red-600 hover:bg-red-500/10 text-xs font-semibold transition-all cursor-pointer"
                    title="Reject"
                  >
                    <X className="w-4 h-4" />
                  </button>
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 lg:grid-cols-4 gap-6">
        {approvedMembers.map((member) => (
          <LuxuryCard key={member.id} className="text-center space-y-3 flex flex-col items-center justify-between">
            <div className="relative">
              <img
                src={member.avatar_url || `https://api.dicebear.com/7.x/avataaars/svg?seed=${member.full_name}`}
                alt={member.full_name}
                className="w-20 h-20 rounded-full border-2 border-[var(--border-gold)] object-cover gold-shadow"
              />
            ...
            {member.mobile_number && (
              <p className="text-xs text-[var(--text-muted)] font-mono flex items-center justify-center gap-1 pt-2 border-t border-[var(--border-subtle)] w-full">
